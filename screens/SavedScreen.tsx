...
import { userSavedPropertiesQuery } from "../hooks/queries/useSavedPropertiesQuery";
import { Loading } from "../components/Loading";

export const SavedScreen = () => {
  const [activeIndex, setActiveIndex] = useState<number>(0);
  const { user } = useUser();
  const savedProperties = userSavedPropertiesQuery();
    ...
    if (activeIndex === 0) {
      ...
      return (
        <>
          ...
          {!user && (
            <SignInAndSignUpButtons style={styles.signInAndSignUpContainer} />
          )}
        </>
      );
    }
